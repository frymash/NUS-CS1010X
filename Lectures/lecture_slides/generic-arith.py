...
def install_number_package():
    def make_number(x):
        return tag(x)
    def tag(x):
        return attach_tag("number", x)
    # add,sub,mul,div: (RepNum, RepNum) -> Generic-OrdNum
    def add(x,y):
    ...
    def div(x,y):
        return make_number(x/y)
    def negate(x): # (RepNum) -> Generic-OrdNum
        return make_number(-x)
    def is_zero(x): # (RepNum) -> Py-Bool
        return x == 0
    def is_equal(x,y): # (RepNum, RepNum) -> Py-Bool
        return x == y
    put("make","number", make_number)
    put("negate",("number",), negate)
    put("is_zero",("number",), is_zero)
    put("add",("number","number"), add)
    put("sub",("number","number"), sub)
    put("mul",("number","number"), mul)
    put("div",("number","number"), div)
    put("is_equal",("number","number"), is_equal)
...
def install_complex_package():
    ...
    def div_com(x,y):
        ...
        x_times_com_conj = mul_com(x, com_conj)
        y_times_com_conj = mul_com(y, com_conj)
        return make_com(div(real(x_times_com_conj), real(y_times_com_conj)),
                        div(imag(x_times_com_conj), real(y_times_com_conj)));
    def complex_conjugate(y):
        return make_com(real(x),
